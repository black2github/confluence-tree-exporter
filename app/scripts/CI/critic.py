...
# Длина ключа проекта — до 20 символов, как в app/color_map.TASK_ID_RE.
# Маски ОБЯЗАНЫ совпадать: color_map извлекает id из истории и пишет маркеры,
# critic их разбирает. Пока здесь стояло {1,9}, миграция выдавала разметку,
# которую собственный critic читать отказывался: apply-all по странице с
# DBOCORPESPLN-61419 (ключ проекта 12 символов) падал с «незакрытый или
# некорректный маркер {++» (инцидент 2026-09-02).
TASK_ID_PATTERN = r"(?:[A-Z][A-Z0-9]{1,19}-\d+|UNKNOWN-[0-9a-fA-F]+)"
...
def file_mentions_task(path: Path, task_id: str) -> bool:
    ...
    разбора против 0,04 с при отборе, а хронология этапа 4 делает такой проход
