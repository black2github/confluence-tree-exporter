...
def _resolve_page_content(page_data: Dict, include_unapproved: bool, critic: bool,
    ...
    if critic:
        ...
        # Третий случай того же класса: служебный столбец таблицы без имени `status`.
        # Нотация опознаёт табличную правку по имени столбца, поэтому apply/reject
        # такую таблицу пропускают целиком — разметка ±ID доезжает до ПРОМ-среза,
        # а строки «на удаление» не удаляются (инцидент 2026-09-06).
        unnamed = find_unnamed_status_column(content)
        for cell in unnamed:
            logger.warning(
                "  ⚠ '%s': строка %d — служебная ячейка %s%s в таблице без столбца "
                "status; apply/reject эту таблицу пропустят "
                "(починка: run-repair --name-status-column)",
                name, cell["line"], cell["sign"], cell["task"])
        ...
        if critic_acc is not None:
            ...
            for cell in unnamed:
                critic_acc["unnamed_status"].append(
                    {"page": name, "line": cell["line"],
                     "sign": cell["sign"], "task": cell["task"]})
            for blk in in_code:
