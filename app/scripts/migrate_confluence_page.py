...
def write_md_file(
    ...
    frontmatter_str = yaml.dump(
        ...
        sort_keys=False,
        # Длинное значение НЕ сворачивать на вторую строку. Ширина PyYAML по
        # умолчанию — 80 символов; для кириллицы это ~150 байт, и длинный title
        # разрывался с переносом продолжения. YAML при этом валиден и данные целы,
        # но всё, что читает файл построчно (grep, инвентаризация выгрузки), видело
        # обрезанный заголовок с незакрытой кавычкой — и одинаковые «головы» разных
        # страниц выглядели дублями (инцидент 2026-08-23).
        width=10 ** 9,
    )
